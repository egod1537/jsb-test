...
void TrimPanel::Draw(TrimPanelProps props) {
  UI::VerticalLayout()
      .Spacing(TrimLayoutSpacing)
          [+MakeTrimInputPanel(props.request)
              + UI::HorizontalLayout().Spacing(
                  8.0F)[+UI::Button("RunIC Trim")
                            .Width(TrimButtonWidth)
                            .Enabled(props.canRequestTrim)
                            .OnAction(props.requestRunICTrim)
                        + UI::Button("Current State Trim")
                            .Width(TrimButtonWidth)
                            .Enabled(props.canRequestTrim)
                            .OnAction(props.requestCurrentStateTrim)]
              + MakeTrimRequestSummary(props.request) + UI::Space(6.0F)
              + UI::FoldOut("Result").Open(
                  props.resultOpen)[MakeTrimResultContent(
                  props.result, props.hasResult)]
              + UI::FoldOut("Residual")
                  .Open(props.residualOpen)[MakeTrimResidualContent(
                      props.result)]]
      .Render();
}
} // namespace gui
